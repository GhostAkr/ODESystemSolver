...
"""
    output_by_argument_dp()

Output Dormand - Prince calculation results as a tables. Argument of each table is `arg`. 
Table values: total count of steps, count of rejected steps, total calculation time, global 
error.

# Arguments
`arg::ResultArgument`: parameter which should be the argument of tables.
"""
function output_by_argument_dp(arg::ResultArgument)
    t_limits = (0.1, 4.1)
    initial_steps = [step for step in 0.01:0.001:0.2]
    initial_val = exact_sol(t_limits[1])
    tol = 1e-4
    ...
    for param in param_vec
        ...
        if arg == InitStep
            res_tuple = solve_nested_rk(f, t_limits, param, initial_val, tol, max_stage, 
                coeffs[1], coeffs[2], coeffs[3], coeffs[4], facs[1], facmins[1], 
                facmaxes[1])
        elseif arg == Fac
            res_tuple = solve_nested_rk(f, t_limits, initial_steps[1], initial_val, tol, max_stage, 
                coeffs[1], coeffs[2], coeffs[3], coeffs[4], param, facmins[1], facmaxes[1])
        elseif arg == FacMax
            res_tuple = solve_nested_rk(f, t_limits, initial_steps[1], initial_val, tol, max_stage, 
                coeffs[1], coeffs[2], coeffs[3], coeffs[4], facs[1], facmins[1], param)
        elseif arg == FacMin
            res_tuple = solve_nested_rk(f, t_limits, initial_steps[1], initial_val, tol, max_stage, 
                coeffs[1], coeffs[2], coeffs[3], coeffs[4], facs[1], param, facmaxes[1])
        end
...
Perform calculations with Dormand - Prince method and output results to external files.
"""
function output_results_dp()

    @info "Solving for different initial steps..."
    output_by_argument_dp(InitStep)
    
    @info "Solving for different facs..."
    output_by_argument_dp(Fac)

    @info "Solving for different facmaxes..."
    output_by_argument_dp(FacMax)
